...
const T = {
  en: {
    ...
    tour: { skip: "Skip", back: "Back", next: "Next", done: "Get started", reopen: "Guide",
      steps: [
        { h: "Welcome to BijliHisaab", p: "Estimate your electricity bill in under a minute — and see exactly why it costs what it does. Free, no sign-in, and everything stays on your device." },
        { h: "Start with your units", p: "Type the units from your bill or tap a common amount, then pick your company and consumer type. Not sure? The on-screen note shows where to find each value." },
        { h: "See the ‘why’, not just a number", p: "Every estimate breaks down into energy, fuel adjustment, taxes and fixed charges — and warns you about the slab traps that quietly inflate Pakistani bills." },
        { h: "Find savings, switch language", p: "Every estimate comes with tailored, most-impactful-first tips to bring your bill down. Switch between English and اردو anytime from the top-right." },
      ] },
  },
  ur: {
    dir: "rtl",
    ...
    tour: { skip: "چھوڑیں", back: "پیچھے", next: "آگے", done: "شروع کریں", reopen: "گائیڈ",
      steps: [
        { h: "بجلی حساب میں خوش آمدید", p: "ایک منٹ سے کم میں اپنے بجلی بل کا تخمینہ لگائیں — اور دیکھیں یہ اِتنا کیوں ہے۔ مفت، بغیر سائن اِن، اور سب کچھ آپ کے آلے پر رہتا ہے۔" },
        { h: "یونٹ سے آغاز کریں", p: "بل کے یونٹ درج کریں یا کوئی عام مقدار دبائیں، پھر اپنی کمپنی اور صارف کی قسم چنیں۔ معلوم نہیں؟ اسکرین پر نوٹ بتاتا ہے ہر قدر کہاں ملے گی۔" },
        { h: "صرف رقم نہیں، ‘کیوں’ دیکھیں", p: "ہر تخمینہ توانائی، فیول ایڈجسٹمنٹ، ٹیکس اور فکسڈ چارجز میں تقسیم ہوتا ہے — اور اُن سلیب جالوں سے خبردار کرتا ہے جو پاکستانی بل چپکے سے بڑھا دیتے ہیں۔" },
        { h: "بچت تلاش کریں، زبان بدلیں", p: "ہر تخمینے کے ساتھ آپ کے بل کے مطابق، سب سے مؤثر پہلے، بچت کے مشورے ملتے ہیں۔ اوپر دائیں سے کسی بھی وقت English اور اردو کے درمیان بدلیں۔" },
      ] },
  },
};
...
function Walkthrough({ open, onClose }) {
  ...
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(20,20,20,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} dir={dir}
        style={{ width: "100%", maxWidth: 460, background: "var(--paper)", border: "1px solid var(--ink)" }}>
        <div style={{ padding: "26px 26px 22px" }}>
          <div className="flex items-center justify-between">
            <span className="ww-num ww-label" style={{ color: "var(--accent)" }}>
              {String(i + 1).padStart(2, "0")} / {String(steps.length).padStart(2, "0")}
            </span>
            <button onClick={onClose} className="ww-navlink" style={{ fontSize: 13 }}>{t.tour.skip}</button>
          </div>
          <h2 style={{ marginTop: 16, fontSize: 24, fontWeight: 700, letterSpacing: dir === "rtl" ? "0" : "-0.03em", lineHeight: 1.1 }}>{step.h}</h2>
          <p style={{ marginTop: 12, fontSize: 15, color: "var(--g1)", lineHeight: 1.65 }}>{step.p}</p>
        </div>

        {/* progress rule */}
        <div style={{ display: "flex", height: 3, background: "var(--rule)" }}>
          {steps.map((_, n) => (
            <div key={n} style={{ flex: 1, background: n <= i ? "var(--accent)" : "transparent", borderInlineEnd: n < steps.length - 1 ? "1px solid var(--paper)" : "none", transition: "background .3s" }} />
          ))}
        </div>

        <div className="flex items-center justify-between" style={{ padding: "16px 26px", borderTop: "1px solid var(--rule)" }}>
          <button className="ww-btn-ghost" style={{ padding: "10px 16px", opacity: i === 0 ? 0.35 : 1, pointerEvents: i === 0 ? "none" : "auto" }}
            onClick={() => setI((v) => Math.max(0, v - 1))}>{t.tour.back}</button>
          <button className="ww-btn" style={{ padding: "10px 20px" }}
            onClick={() => (last ? onClose() : setI((v) => v + 1))}>{last ? t.tour.done : t.tour.next}</button>
        </div>
      </div>
    </div>
  );
        ...
        <header style={{ position: "sticky", top: 0, zIndex: 20, background: "#fff", borderBottom: "1px solid var(--ink)" }}>
          <div className="ww-wrap flex items-center justify-between" style={{ height: 64, gap: 16 }}>
            ...
            <div className="flex items-center" style={{ gap: 20 }}>
              ...
              <button onClick={() => setTourOpen(true)} title={t.tour.reopen} aria-label={t.tour.reopen}
                style={{ display: "grid", placeItems: "center", width: 30, height: 30, border: "1px solid var(--rule)", background: "#fff", cursor: "pointer", color: "var(--ink)" }}>
                <HelpCircle size={16} />
              </button>
